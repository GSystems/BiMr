...
public class TweetScheduleFacadeBean implements TweetScheduleFacade {
	...
	private void createAndPersistModelsForHotspot(List<HotspotDTO> hotspots) {
		Map<String, List<Model>> allModelsForHotspot = rdfFacade.createModelsForHotspots(hotspots);
		for (Map.Entry<String, List<Model>> entry : allModelsForHotspot.entrySet()) {
			rdfFacade.persistAllModelsForAHotspot(entry.getValue());
		}
	}
	...
	private List<HotspotDTO> filterTweetsAndGenerateHotspots(List<TweetDTO> tweets) {
		...
		for (TweetDTO tweet : tweets) {
			...
			if (hotspot.getBirdSpecies() != null && tweet.getLatitude() != null || hotspot.getCity() != null) {
	...
	public List<TweetDTO> mockTweets() {
		...
		tweetDTO110.getUser().setId("1110");
		...
		tweetDTO210.getUser().setId("1101");
		...
		tweetDTO310.getUser().setId("3110");
		...
		tweetDTO410.getUser().setId("1109");
